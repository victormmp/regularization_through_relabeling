...
x, y = make_moons(n_samples=samples, random_state=random_seed, noise=0.25)


for c in range(2):
    cl = y == c
    plt.scatter(x[cl, 0], x[cl, 1],s=20, edgecolor='black')
...
for c in range(2):
    cl = y_train == c
    plt.scatter(x_train[cl, 0], x_train[cl, 1],s=20, edgecolor='k')
    
plot_plain_separator(model, x, save=fr'{"_".join(dataset_name.lower().split())}_nao_ajustada', grid_range=(-3, 3))
...
plot_plain_separator(model, x, save=fr'{"_".join(dataset_name.lower().split())}_ajustada', grid_range=(-3, 3))
